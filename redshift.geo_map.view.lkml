view: rs_logrecno_bg_map {
    ...
    derived_table: {
      ...
      SELECT
        ...
        tract,
        blkgrp,
        CASE WHEN SUBSTRING(SUBSTRING(geo.geoid::varchar, 8, 11),1,1) = '0' THEN SUBSTRING(geo.geoid::varchar, 9, 10) ELSE SUBSTRING(geo.geoid::varchar, 8, 11) END  AS  geoid11, --SUBSTRING(geo.geoid, 8, 11) as geoid11,
        geo.geoid,
        trim(' ' from CASE
          WHEN sumlevel = '140'
      ...
      FROM
        looker_demographic.geo2015 as geo
      LEFT JOIN looker_demographic.block_group_attribs as bg on (SUBSTRING(geo.GEOID::varchar, 8, 12)::varchar = bg.geoid::varchar AND geo.SUMLEVEL = '150')
      LEFT JOIN looker_demographic.block_group_attribs as tr on (SUBSTRING(geo.GEOID::varchar, 8, 11)::varchar = SUBSTRING(tr.geoid::varchar, 1, 11)::varchar AND geo.SUMLEVEL = '140')
      WHERE
        sumlevel in ('140','150')

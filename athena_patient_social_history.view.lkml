...
LEFT JOIN (
    SELECT chart_id, social_history_name, social_history_answer AS answer, created_datetime, ROW_NUMBER() OVER (PARTITION BY chart_id ORDER BY created_datetime DESC) AS rownum
            FROM athena.patientsocialhistory
    ...
    LEFT JOIN (
      SELECT chart_id, social_history_name, INITCAP(social_history_answer) AS answer, created_datetime, ROW_NUMBER() OVER (PARTITION BY chart_id ORDER BY created_datetime DESC) AS rownum
              FROM athena.patientsocialhistory
      ...
      SELECT chart_id, social_history_name,
        ...
        END
         ...
         created_datetime, ROW_NUMBER() OVER (PARTITION BY chart_id ORDER BY created_datetime DESC) AS rownum
              FROM athena.patientsocialhistory
        WHERE social_history_name LIKE '%Has lack of transportation kept you from medical appointments, meetings, work, or from getting things needed for daily living?' AND
    ...
    LEFT JOIN (
      SELECT chart_id, social_history_name, LOWER(social_history_answer) AS answer, created_datetime, ROW_NUMBER() OVER (PARTITION BY chart_id ORDER BY created_datetime DESC) AS rownum
              FROM athena.patientsocialhistory
...
LEFT JOIN (
        SELECT chart_id, social_history_name, INITCAP(social_history_answer) AS answer, created_datetime, ROW_NUMBER() OVER (PARTITION BY chart_id ORDER BY created_datetime DESC) AS rownum
          ...
          social_history_name = 'Has it ever happened within the past 12 months that the food you bought just didn’t last, and you didn’t have money to get more?'
...
CASE WHEN created_datetime <= '2019-12-13 14:25:00' AND INITCAP(social_history_answer) = 'Yes' THEN 'No'
        ...
        END
         ...
         created_datetime, ROW_NUMBER() OVER (PARTITION BY chart_id ORDER BY created_datetime DESC) AS rownum
              FROM athena.patientsocialhistory
        WHERE social_history_name LIKE '%Within the past 12 months we worried that our food would run out before we got money to buy more.' AND
...
ORDER BY base.chart_id;  ;;
      ...
      sql_trigger_value: SELECT COUNT(*) FROM athena.patientsocialhistory ;;
      indexes: ["chart_id"]
    }
    ...
    dimension: cost_concerns_flag {
      type: yesno
      sql: ${cost_concerns} <> 'No' AND ${cost_concerns} <> 'Choose Not To social_history_answer This Question'
